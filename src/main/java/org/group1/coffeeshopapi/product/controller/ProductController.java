...
public class ProductController {
    ...
    public ApiResponse<ProductResponse> update(@PathVariable UUID id, @Valid @RequestBody UpdateProductRequest request) {
        return ApiResponse.of(HttpStatus.OK, "Product updated successfully.", productService.update(id, request));
    }
    ...
    public ApiResponse<ProductResponse> setDiscount(
        ...
        return ApiResponse.of(HttpStatus.OK, "Product discount set successfully.", productService.setDiscount(id, request));
    }

    @DeleteMapping("/{id}/discount")
    public ApiResponse<ProductResponse> clearDiscount(@PathVariable UUID id) {
        return ApiResponse.of(HttpStatus.OK, "Product discount cleared successfully.", productService.clearDiscount(id));
    }

    @PostMapping(value = "/{id}/image", consumes = "multipart/form-data")
    public ApiResponse<ProductResponse> uploadImage(@PathVariable UUID id, @RequestParam("file") MultipartFile file) {
        return ApiResponse.of(HttpStatus.OK, "Product image uploaded successfully.", productService.uploadImage(id, file));
    }

    @DeleteMapping("/{id}/image")
    public ApiResponse<ProductResponse> removeImage(@PathVariable UUID id) {
        return ApiResponse.of(HttpStatus.OK, "Product image removed successfully.", productService.removeImage(id));
    }
    ...
    public ApiResponse<ProductImportResponse> importExcel(@RequestParam("file") MultipartFile file) {
        ProductImportResponse response = productService.importFromExcel(file);
